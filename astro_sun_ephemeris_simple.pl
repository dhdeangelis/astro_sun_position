...
my $initDate = date(qq($epheYear-06-20));
...
foreach my $zangle (@zangles) {
	...
	# if hour angle is invalid, for example if Sun does not set, the above calculation retrieves a complex number
	# in that case, check if the result is a complex number and if true just go to next value in sequence
	next if Im($h) != 0;
